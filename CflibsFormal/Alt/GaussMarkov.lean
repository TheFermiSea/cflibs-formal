...
* `weight_sq_ge_noiseGain` — **the deterministic algebraic core** `∑ₖwₖ² ≤ ∑ₖaₖ²` for any unbiased
  ...
  vanishes (`∑ₖwₖaₖ = ∑ₖwₖ² = 1/SS_E`). Reuses `ErrorBudget.olsSlope_noise_gain`.
...
`Alt.OLSVariance`; its deterministic kernel is `ErrorBudget.olsSlope_noise_gain` (`∑wₖ² = 1/SS_E`).
-/
...
is the special case `a = olsWeight E` (via `ErrorBudget.olsSlope_eq_centered`); BLUE optimality
...
(`ErrorBudget.centered_sum_zero`, `centered_mul_self`), so OLS is admissible in the BLUE class. -/
...
the OLS noise gain is `∑wₖ² = 1/SS_E` (`ErrorBudget.olsSlope_noise_gain`), the cross term is
