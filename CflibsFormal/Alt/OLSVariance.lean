...
theorem expectation_const_add_weightedNoise (w : ι → ℝ) (c : ℝ) (ε : ι → Ω → ℝ)
    (hL2 : ∀ k, MemLp (ε k) 2 μ) (hmean0 : ∀ k, μ[ε k] = 0) :
    μ[fun ω => c + ∑ k, w k * ε k ω] = c := by
  have hint : ∀ k, Integrable (fun ω => w k * ε k ω) μ :=
    fun k => ((hL2 k).const_mul (w k)).integrable (by norm_num)
  have hSint : Integrable (fun ω => ∑ k, w k * ε k ω) μ :=
    (memLp_finsetSum Finset.univ (fun k _ => (hL2 k).const_mul (w k))).integrable (by norm_num)
  rw [integral_add (integrable_const c) hSint,
...
theorem olsSlope_unbiased [Nonempty ι] (E : ι → ℝ) (α β : ℝ) (ε : ι → Ω → ℝ)
  ...
  exact expectation_const_add_weightedNoise (olsWeight E) β ε hL2 hmean0

/-- **Slope variance as the noise gain** `Var(β̂) = σ²·∑ₖ wₖ²`. The `w = olsWeight E` instance of
`variance_const_add_weightedNoise`, after rewriting `β̂` to `β + ∑ₖ wₖ·εₖ`. The `card`-free form
