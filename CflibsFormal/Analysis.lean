...
* `coupledFixedPoint_lt_of_map_lt` / `coupledFixedPoint_le_of_map_le` — monotone fixed-point
  comparative statics (antitone lower map dominated pointwise ⇒ fixed points ordered), the
  order-theoretic core under the multi-element ionization coupling in `MatrixIonizationCoupling`.
