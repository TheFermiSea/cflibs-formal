# cflibs-formal — Development Specification (v0.6, 2026-09-24)
...
| D14 | Primary Worker and continuous operation | **Qwen3.8-27B on all three nodes; continuous proof queue on ai-proxy** (owner, 2026-09-24: "if both produce similar quality but one is significantly faster, go with the faster"). Evidence and its scope: on the three C3 statements Qwen and a correctly served Leanstral 1.5 both closed 3/3 at similar token cost, Qwen in 20–47 min against Leanstral's 4.5–8.5 h (03 §7.5); three algebraic statements from one family with a Claude planner, so this ranks speed, not ability on harder targets. Leanstral's launchers, template and drop-ins stay on infer-02/03 (service disabled) for a second opinion. Planner stays Claude (`sonnet`, D8). Queue, verifier and unit: `tools/openprover/queue/` |
| D15 | Scope-tag semantics | **Two axes, publish the weaker** (owner, 2026-09-24): definitions that encode a physical model carry a MODEL tag (e.g. `lineIntensity` REDUCED, `selfAbsorptionFactor` APPROXIMATION); theorems carry a RELATION tag (how exactly they hold for the model they are stated over); the PUBLISHED scope is the weaker of the two, computed by ScopeCheck; PURE-MATH theorems exempt. Resolves audit RF-27's EXACT ambiguity (about 33 results depended on it). Implemented in the P0 batch (branch `fix/p0-honest-scope`) |
| D16 | Certificate IDs | **Tied to purpose** (owner, 2026-09-24): C8 = Stark–Saha n_e consistency, now the error-bar form with a provable refusal (audit RF-20/FT-06); C11 = outer-loop convergence, now the stop certificate for the loop `iterative.py` runs (FT-01). Neither ID ever shipped, so nothing breaks; the frontier-12 dossier is updated to match. New purposes (grouped design rank C1g/C2g, C4σ, the refuse-to-report policy) take C15 onward. Mirror in CF-LIBS-improved's `LEAN_CERTIFICATE_DEFS` |
| D17 | Partition-function level cutoff | **Continuous policies only; Hummer–Mihalas occupation probabilities as the physics candidate, a fixed cutoff at the unperturbed ionization energy as the baseline, PAS chooses between them; the n_e-dependent sharp cutoff is retired** (owner, 2026-09-24, conditional on literature, confirmed by `docs/research/ipd-and-level-cutoff-2026-09-24.md`: a sharp cutoff at χ − Δχ(n_e) makes U discontinuous in n_e and voids every Lipschitz/contraction result; Hummer & Mihalas 1988 treat truncation as physically distinct from the lowering of the ionization potential). Caveat recorded: the Hubeny–Hummer–Lanz 1994 fit is outside its validated range at 0.5 eV with n_e ≥ 1e17 and at 1 eV with 1e18. The spec generalizes U to a weighted level sum so both policies are instances (audit FT-05) |
| D18 | Ionization-potential depression model | **Debye–Hückel with electron and ion screening (DH-ei) as the pipeline default, one function shared by forward and inverse, 2Δχ at the II→III edge; Stewart–Pyatt as a sensitivity arm** (owner, 2026-09-24, from the research memo: DH-ei is the standard thermodynamic result and the form every retrieved LIBS paper uses; Stewart–Pyatt's known failures are at solid density, but it sits in its crossover regime at LIBS conditions and falls outside the √n_e family). The formal IPD results keep the coefficient abstract (b ≥ 0; audit FT-02); the ⅔ coefficient question raised by Crowley 2014 is recorded as open |
| D19 | Radiation conventions | **Frequency form canonical, with a proved change-of-variables bridge to the wavelength form** (owner, 2026-09-24): Einstein relations, Kirchhoff's law and σ₀ = (λ²/8π)(g_u/g_l)A·φ(ν) with ∫φ dν = 1 stated in ν; one lemma gives the λ⁴/8πc wavelength form the pipeline uses; the stimulated-emission factor (1 − e^(−hν/kT)) included in both. Constants checked against standard references before FT-14 is stated |
...
| v0.6 | 2026-09-24 | Owner decisions D15–D19 (scope-tag semantics, certificate IDs, cutoff policy, IPD model, radiation conventions) after the deep audit; IPD and level-cutoff literature memo (`docs/research/ipd-and-level-cutoff-2026-09-24.md`) |
| v0.5 | 2026-09-24 | D14: Qwen3.8-27B primary Worker on all three nodes; continuous proof queue on ai-proxy (`tools/openprover/queue/`: supervisor, `verify.py` independent re-verification falsified on seven cases, systemd unit), seeded with Frontier 02 and 07 (the Phase-0 Worker exit criterion). PR #6 merged (`fb1681d`) |
| v0.4.3 | 2026-09-23 | Leanstral matched rerun **3/3** (03 §7.5): Mistral 1.5 chat template (the GGUF's 2603 one 500'd four Worker calls), card sampling, equal reasoning budget, Worker usage summed over turns, equal-token budget; wall-clock 4.5–8.5 h vs Qwen's 20–47 min. Fleet fixes (04 §10.4): infer-01 down after a pve1 reboot (`onboot` now set), infer-03 vCPUs never pinned, infer-02 model split unevenly across NUMA nodes. Mistral Vibe arm started. Correction: v0.4.2 said PR #6 had "landed on `main`" before it was merged |
