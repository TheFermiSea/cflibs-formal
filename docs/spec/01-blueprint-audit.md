...
| `integral_comp_mul_left`, `integral_comp_add_right` | VERIFIED names, mixed scope | `integral_comp_mul_left` exists for the whole-line Bochner integral (`Measure/Haar/NormedSpace.lean:152`); the `integral_comp_add_right` hit is the **interval**-integral version (`IntervalIntegral/Basic.lean:954`). The whole-line shift lemma's exact name must be grepped at implementation time (UNVERIFIED here) |
