...
| 8 | Pre-registration | `scripts/prereg.sh freeze`; `scripts/prereg.sh audit --results CflibsFormal/<Module>.lean <prereg>` (the `--results` scope is required; the default scope does not include Lean files) | statement retreat after the fact | statements never registered |
...
| Pre-registration presence | every PR adding a new module | no `docs/preregistrations/*.md` whose scoped audit prints `PASS  FROZEN` references the module (statements are then labelled exploratory in the tag column) |
| Red-team record | every change to `.claude/agents/lean-statement-audit.md` (including the Phase-0 change that removes its `Write, Edit` tools and adds the drift-class field) | no `docs/spec/redteam/<date>.md` newer than the change |
...
1. Pre-registration audit prints `PASS  FROZEN` with `--results` scoped to the new module, and the freeze predates the Lean file.
2. The Target-Reviewer verdict on the frozen statement is recorded and is `passed` (with the drift-class field empty).
...
- Lemma names marked "grep at implementation" are not to be cited in docstrings until grepped. When
  grepping mathlib, remember two ways a plain `rg "theorem <name>"` under-reports: names generated by
  `to_additive` (e.g. `integral_sub_right_eq_self`) and `abbrev`/`alias` declarations (e.g.
  `Matrix.mulVecLin`); search for the bare name and check for deprecation aliases
  (`integral_finset_sum` is deprecated in favour of `integral_finsetSum`).
