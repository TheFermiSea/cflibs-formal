...
|---|---|---|---|
| 01 M1–M3 ✅ | `profiledResidual_two_closed_form` (Φ₂ closed form) + `profiledT_onManifold_unique` (general-`m` on-manifold `=0 ⟺ T=T0`, two-line a corollary) + `joint_onManifold_unique` (full `(T,N)` argmin uniqueness) — **DONE** (`NonlinearLeastSquares.lean`, PURE-MATH + EXACT/Ciucci 1999; exact-fit route reuses `temperature_identifiability`, no `∑_{i<j}` Lagrange machinery needed) | `NonlinearLeastSquares.lean` | PURE-MATH + EXACT |
