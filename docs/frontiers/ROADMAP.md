...
- **04 — Outer T-iteration of the full loop** (`04-outer-t-iteration.md`). *Obstacle:* no self-map `Φ` on T
  ...
  **combined Saha–Boltzmann slope** (Model B) which is the only non-degenerate loop (§2)
  *[2026-09-24: Model B is non-degenerate only because its Saha offset is frozen at a reference
  `T`; with the consistent offset `log S(T)` the composite map is constant (audit finding
  INV-01)]*. *First milestone (A):*
...
*[2026-09-25] The "Scope tag" columns below record single-axis tags at landing. Since the owner
decision of 2026-09-24 (`docs/conventions.md` §8) a theorem's row tag is its relation tag, and the
tag to quote for physics is the published one in `docs/scope-published.tsv`. Several rows listed
EXACT here now publish REDUCED through the `lineIntensity` model: `profiledT_onManifold_unique`,
`profiledT_offManifold_unique`, `joint_onManifold_unique` (01) and `olsDensity_aliasing_A` (05).*
...
| 04 M2→M6 ✅ | `neLeg_mapsTo`; **`combinedSahaBoltzmannSlope` + `combinedSlope_offset_lipschitz` (crux)** + `combinedSlopeTempUpdate(_lipschitz)`; `outerLoop_contracts` headline (new module `OuterLoopModelB.lean`, REDUCED-conditional on the `L₁·L₂<1` gate). *[2026-09-24: a frozen-offset, fixed-stage-ratio model: with the consistent offset `log S(T)` the composite is constant (INV-01); the gate evaluates to 6·10³–5·10⁴ on realistic Ti-like data (INV-03), so it is not a usable runtime certificate; it does not model `iterative.py`; no end-to-end witness exists. See `OuterLoopModelB` and `docs/SOLVER_FORMALIZATION_GAPS.md` item 6.]* | `ErrorBudget.lean` / `SahaEquilibrium.lean` / `OuterLoopModelB.lean` | REDUCED |
...
| 04 M8 ✅ (abstract) | joint `(T,n_e)` 2-D map: `jointOuterMap` + `_mapsTo`/`_contraction` (row-sum gate `max(a+b,c+d)<1`, max metric)/`_geometric_error`/`jointOuterContraction_box` (∃! box fixed point + joint convergence). Multi-element physics instantiation deferred (import-DAG: belongs in a higher module). *[2026-09-24: the unweighted row-sum gate is not unit-invariant: rescaling `n_e` rescales the off-diagonal terms, and in eV / cm⁻³ units it always fails (audit finding PS-10). The unit-invariant condition is `a < 1 ∧ d < 1 ∧ bc < (1−a)(1−d)`; not formalized.]* | DONE (spine) | PURE-MATH |
...
|---|---|---|
| 08 Non-LTE / collisional-radiative kinetics | `08-nonlte-kinetics.md` (reviewed: FIXED) | **A-milestones LANDED 2026-07-09** (`NonLTEKinetics.lean`, audit PASS): `departureCoeff`/`two_level_balance` ladder, density-form sandwich + strict monotonicity + LTE limit `b→1`, McWhirter rate-ratio `⇒ b ≥ 10/11`, and the crown bridge — `nonlte_ordinate_shift` + `abs_log_departure_le` feeding `nonlte_temp_error`/`nonlte_density_error` through the existing hetero-δ chain (needed promoting `ErrorBudget.log_lip_floor` to public). Refusals honored: no CR rate matrix, no numeric McWhirter bridge. |
| 09 Radiative transfer beyond the uniform slab | `09-radiative-transfer-depth.md` (reviewed: FIXED) | **A-milestones LANDED 2026-07-09** (`RadiativeTransferDepth.lean`, audit PASS): `rtEmergent` N-zone fold with `rtEmergent_single = slabIntensity` and `rtEmergent_two = emergentIntensity` (convention guards), telescoping sandwich + uniform collapse, and the continuous formal solution `rtFormal` (`_const`/`_sandwich`/`rtFormalLinear` Eddington–Barbier exact case). Frequency-angle coupling + scattering source refused per §5. |
| 10 Spatial inversion: Abel / onion peeling | `10-spatial-abel-inversion.md` (reviewed: FIXED) | **A-milestones LANDED 2026-07-09** (`SpatialForward.lean` extension, audit PASS): `peeling_identity` (exact back-substitution), `onionPeel` + exact recovery + triangularity, `peeling_single_step` perturbation inequality, `peeling_amplification(_iSup)` geometric noise bound, `peeling_condition_linfty` (ℓ∞ — spectral κ refused per §5). Continuous Abel stays refused. |
| 11 Stochastic layer | `11-stochastic-layer.md` (reviewed: FIXED) | **A-grade milestones LANDED** (`Alt/StochasticBudget.lean`, audit PASS): `alphaHat` estimator ladder (`_estimator_eq`/`_unbiased`/`_variance_eq` = σ²(1/n+Ē²/SS_E)), `MemLp` bridges, **Chebyshev tail bounds** `olsSlope_chebyshev` + `alphaHat_chebyshev` (first distributional statements in the repo). B-grade ladder **LANDED 2026-07-09** (audit PASS): `temp_tail_transfer` (the crown — `P(|T̂−T| ≥ ε)` via exact event inclusion, no delta method), `density_tail_species`, `composition_tail_union`, and the sub-Gaussian upgrade `olsSlope_subGaussian_tail` (exponential tail under `iIndepFun` + `HasSubgaussianMGF`). |
| 12 Runtime certificates (CF-LIBS-improved bridge) | `12-runtime-certificates.md` (reviewed: FIXED) | **M1 LANDED** (`Certificates.lean`, audit PASS): 12 arithmetic-evaluable certificates, each a `Prop` + thin soundness wrapper re-exporting a proven gate (energy spread, joint rank, conditioning, slope/temp/composition budgets, McWhirter, Saha/damped iteration, known-τ SA, COG distinctness, alias budget) + non-vacuity witnesses. Reference Python mirror at `docs/integration/cflibs_certificates.py` (smoke-tested, 12/12 + negative cases). **M2 INSTALLED in CF-LIBS-improved 2026-07-09** (independent drift-verify PASS: all 12 predicates 1:1 with the Lean twins): `cflibs/inversion/physics/certificates.py` + 30 tests + an additive, default-OFF CLI hook (C7 surfaced from finished results; full suite via the public `evaluate_certificates` API). Certificates OBSERVE, they do not gate. **M3 LANDED 2026-07-09 (second pass, audit PASS)**: oracle `certificates` scenario — Lean Float mirrors of all 12 predicates + negatives in `oracle/Generate.lean` → `fixtures.json` (append-only, prior scenarios byte-identical) → vendored into CF-LIBS-improved as `tests/data/certificates_fixtures.json` + a cross-repo regression test driving the SHIPPED module (48/48; breaks if either repo drifts; CI-covered). Deep wiring: 5/12 certificates auto-evaluate on real runs (C1/C2/C3/C7/C12, flag-gated default-OFF, byte-identical off) with 7 honest documented skips. Rust mirror assessed and DEFERRED (native/cflibs-core has no fit-level computation). **[2026-09-24 deep audit — corrections; the per-certificate ledger is `12-runtime-certificates.md` §0.]** Passing a certificate buys less than this row suggests: C3 adds nothing to C1; C1/C2 certify a pooled single-intercept design, not the per-element fit the solver runs; C7 is necessary, not sufficient, for LTE; C9/C10 certify forward closures at known `Ntot`, not the inverse `(T, n_e)` loop or its `0.5` damping; C12 is `0 ≤ τ` and certifies nothing about the data; C13 holds for the flat kernel on opacity coefficients only. The C4, C6 and C14 witnesses sit at zero-error corners. The `EvaluatorSoundness` bundle is not an informative composition bound at realistic parameters (26–1.42·10³, U-01) and must not be advertised or scored as one. "Breaks if either repo drifts" is false: formal-side changes never reach the vendored fixture copies (G-11). The companion's overhaul review, quoted in audit finding U-14, found `identifiability.py` and `reliability.py` dead in production and called the five auto-evaluated certificates "almost vacuous and ... evaluated on the wrong line set". "Certificates OBSERVE, they do not gate" holds for the inversion CLI hook only; the companion's evolution gate treats C1–C4, C7, C9, C10, C12 and C13 as HARD (frontier 12 §0). Certificate IDs C8 and C11 are reserved for the error-bar Stark–Saha certificate and the stop certificate for the loop `iterative.py` runs (owner decision D16); neither is implemented. |
| 13 2DCOS (gated-delay 2D correlation spectroscopy) | `13-2dcos-formalization.md` | **LANDED** — see `docs/2dcos/master-manuscript.md` Appendix A (`TwoDCOS.lean`); the refuted "Model B" nₑ-elimination / "standardless" claim was audited unsound and removed (`docs/2dcos/ERRATA.md`). |
