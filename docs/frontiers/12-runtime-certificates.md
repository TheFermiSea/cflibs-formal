...
>
> **Certificate IDs (owner decision D16, 2026-09-24; recorded in `docs/spec/README.md` on the
> `docs/formalization-spec` branch).** IDs are tied to purpose. **C8** stays the Stark–Saha `n_e`
> consistency certificate, now in error-bar form with a provable refusal (RF-20 / FT-06). **C11**
> stays the outer-loop convergence certificate, now a stop certificate for the `(T, n_e)` loop
> `iterative.py` runs (RF-17 / FT-01), replacing the Model B product-gate design. Neither is
> implemented, and neither ID ever shipped. New purposes (the grouped design certificates
> C1g/C2g, the statistical C4σ, the refuse-to-report policy) take IDs from C15 onward; the names
> used below are working names. The companion's `LEAN_CERTIFICATE_DEFS` is to mirror this
> numbering.
>
> Published scope: each soundness theorem's published tag (owner decision D15; `docs/conventions.md`
> §8) is in `docs/scope-published.tsv`.
>
...
| C8 | — | not implemented. The ID is reserved (D16) for the error-bar Stark–Saha `n_e` consistency certificate with a provable refusal (RF-20 / FT-06) | the named theorem `stark_saha_lte_consistent` needs exact Stark = Saha equality, so it cannot back a tolerance predicate (U-11, PS-09) | — |
...
| C11 | — | not implemented. The ID is reserved (D16) for a stop certificate for the `(T, n_e)` loop `iterative.py` runs (RF-17 / FT-01) | the 2026-07 design, the Model B product gate `L₁·L₂ < 1`, evaluates to 6·10³–5·10⁴ at realistic parameters, so it cannot pass (INV-03), and Model B freezes the Saha offset at a reference `T` (INV-01) | — |
...
noise on NIST Fe/Cr/Ni level lists, where a fraction error is at most 1. The Fe and Ni bounds both
fall below 1 only when the level lists are truncated at 1 eV and the noise is at most 2 %; with a
3 eV truncation the Ni bound alone falls below 1, in three cells (smallest 0.47), and the Fe bound
never does (U-01, re-run by the verifier; the 3 eV cells are from the later re-run recorded in
`NoiseToComposition`, *Non-vacuity range*). The loose
leg is the partition-function channel. Do not score or advertise the evaluator bundle as a certified
...
pipeline runs (RF-17 / FT-01; it takes the C11 ID under D16) instead of relying on C10 or the
Model B product gate.
...
The repo's own ledger already frames the target: `docs/SOLVER_FORMALIZATION_GAPS.md` Tier 3 — *"Tier 3 — the hypothesis IS the runtime check (wire it; no Lean fix required)"* — and `OuterLoopModelB.lean:71` calls `L₁·L₂ < 1` "the runtime-checkable convergence certificate the solver flag gates on." *[2026-09-24: that `OuterLoopModelB` sentence has been removed; the `outerLoop_contracts` docstring now calls the gate a sufficient convergence condition. The gate evaluates to 6·10³–5·10⁴ at realistic parameters (INV-03), Model B freezes the Saha offset (INV-01), and nothing in `iterative.py` evaluates it (§1.1).]* This dossier turns that scattered intent into **one typed bridge**: a `CflibsFormal/Certificates.lean` where each certificate is a `Prop` of pure float arithmetic paired with a soundness theorem `certificate → guarantee`, plus a reference Python checker mirroring those exact defs — extending the existing `oracle/` + `check_fixtures.py` mechanism, not inventing a new one.
...
| C8 | Stark↔Saha LTE self-consistency | `n_e^Stark`, `n_e^Saha`, `T`, `ΔE` | `\|Δn_e\|/mean ≤ rtol` ∧ `mean ≥ C·√T·ΔE³` | `StarkBroadening.stark_saha_lte_consistent` (`StarkBroadening.lean:189`) ⇒ two independent diagnostics agree + clear McWhirter **[2026-09-24: not implemented. The named theorem needs exact equality `hagree` of the two estimates, so it does not cover the `rtol` predicate (U-11, PS-09). An error-bar C8 is RF-20 / FT-06, and D16 keeps the C8 ID for it]** | `reliability.py:stark_saha_lte_gate:205`; `stark_ne.py:measure_stark_ne:394` | ~~A~~ not landed |
...
- **C12** — `lineIntensity_eq_selfAbsorbedIntensity_div` holds for all `τ≥0`; the only "certificate" is that `τ` is known. **[2026-09-24] Retracted from "implement now".** "Known" is not arithmetic, so the landed predicate is `0 ≤ τ` alone, and the identity holds for every `τ ≥ 0`, including a wrong estimate: C12 certifies nothing about the data (LF-11, U-05, SPC-03). The self-absorption model is also approximate (flat line-centre escape factor applied to integrated intensities; LF-01): `selfAbsorptionFactor` and `selfAbsorbedIntensity` carry the model tag APPROXIMATION. The wrapped identity `lineIntensity_eq_selfAbsorbedIntensity_div` is tagged PURE-MATH (a left-inverse, no physics content). The replacement (FT-13) is a ½-Lipschitz bound on `log SA` plus a `τ`-error propagation lemma whose content sits in an assumed `|τ̂ − τ| ≤ Δ`: a refusal-grade lemma (§5 R7), not a checkable certificate.
...
A stdlib-only module in the style of `oracle/check_fixtures.py`, mirroring each M1 `def` as a `…_certificate(inputs) -> (bool, theorem_name, value)`. Consolidates the three existing partial mirrors (`derived_thresholds.py`, `reliability.py`, `identifiability.py`) behind one certificate protocol so every gate returns the theorem it activated. Attach map: C1/C3→`boltzmann.py:fit`+`reliability.py:temperature_conditioning`; C2→`closed_form.py:_build_design_matrix`/`joint_optimizer.py:optimize`; C4/C5/C6→`derived_thresholds.py`→`line_selection.py`; C7/C8→`lte_validator.py:check_mcwhirter`/`reliability.py:stark_saha_lte_gate`; C9→`saha_boltzmann.py:solve_species_states`; C10→`anderson_solver.py:picard_solve` only *[2026-09-24: not the `0.5` damping of the inverse loop, §1.1]*; C11→`iterative.py` outer loop (log `L₁·L₂` verdict beside `converged`) *[2026-09-24: do not wire; the gate cannot pass at realistic parameters, INV-03 / RF-11. Under D16 the C11 ID goes to the RF-17 / FT-01 stop certificate, whose attach point is this loop; not implemented]*; C12/C13→`self_absorption.py`/`identifiability.py`; C14→`quality.py:per_element_reliability_from_uncertainty`.
...
`native/` crate mirrors the hot certificates (C1, C4, C7, C10) for the real-time path, conformance-checked against the same `fixtures.json`. Strict mode `CFLIBS_NO_FALLBACK` (`docs/SOLVER_FORMALIZATION_GAPS.md`, "How the solver consumes this") already gates solves on proven preconditions and refuses; wire each certificate verdict into that decision and into the M8 per-element reliability flags (`quality.py:downgrade_quality_flag:708`) so a failed certificate names its theorem.
...
2. Certify the loop the pipeline runs: exact reduced-map gain, a damped-Picard window and an a-posteriori residual stop (RF-17 / FT-01). Under D16 this certificate takes the C11 ID. It, not C10 or the Model B product gate, is what can speak to the `0.5` damping.
...
4. Land C8 on error bars (RF-20 / FT-06; the C8 ID per D16) and the statistical C4σ (FT-12; a new purpose, so an ID from C15 onward per D16), each with a refusal note for its assumed inputs.
